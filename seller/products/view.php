...
$slug = Database::query("SELECT slug FROM products WHERE id=? AND seller_id=? LIMIT 1", [$id, Session::getUserId()])->fetchColumn();
if (!$slug){ header('Location:/seller/products/'); exit; }
header('Location:/product/'.$slug); exit;
