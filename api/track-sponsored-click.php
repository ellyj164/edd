...
try {
    ...
    // Get the sponsored product ID for analytics
    $stmt = $db->prepare("
        SELECT id FROM sponsored_products 
        WHERE product_id = ? 
        AND status = 'active'
        AND sponsored_until >= NOW()
        LIMIT 1
    ");
    $stmt->execute([$productId]);
    $sponsoredProduct = $stmt->fetch();
    
    // Log the click for analytics (optional)
    if ($sponsoredProduct) {
        $stmt = $db->prepare("
            INSERT INTO sponsored_product_analytics 
            (sponsored_product_id, event_type, user_id, session_id, ip_address, created_at) 
            VALUES (?, 'click', ?, ?, ?, NOW())
        ");
        ...
        try {
            $stmt->execute([$sponsoredProduct['id'], $userId, $sessionId, $ipAddress]);
        } catch (Exception $e) {
            // Analytics logging failed, that's okay
